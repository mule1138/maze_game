...
describe('math_lib', () => {
    ...
    describe('calcSlopeFromHeading', () => {
        ...
        it('should calculate slope for non-edge-case headings', () => {
            let slope = trimDecimalNumber(MathLib.calcSlopeFromHeading(26.5651), 4);
            assert.equal(slope, -2.0);
            slope = trimDecimalNumber(MathLib.calcSlopeFromHeading(161.5651), 4);
            assert.equal(slope, 3.0);
            slope = trimDecimalNumber(MathLib.calcSlopeFromHeading(333.435), 4);
            assert.equal(slope, 2.0);
            slope = trimDecimalNumber(MathLib.calcSlopeFromHeading(198.435), 4);
            assert.equal(slope, -3.0);
        });
    });
});
